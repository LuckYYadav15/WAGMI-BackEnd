...
const categoryWiseEP = db.category_wise_e_p;
const Year = db.year;
...
// View All categoryWiseEP data
exports.index = (req, res) => {
    categoryWiseEP.findAll({})
        .then(category_wise_f_ep => {
            res.send({
                message: "List of all category_wise_f_ep",
                data: category_wise_f_ep,
            });
        })
        .catch(err => {
            res.status(500).send({
...
// Create categoryWiseEP
exports.create = (req, res) => {
    categoryWiseEP.create({
        ...
        .then(category_wise_e_p => {
            res.status(200).send({
                message: "categoryWiseEP Added Successfully!",
                data: category_wise_e_p,
            });
        })
        .catch(err => {
            res.status(500).send({
...
// View Individual categoryWiseEP
exports.findByPk = (req, res) => {
    categoryWiseEP.findByPk(req.params.categoryWiseEPId)
        .then(category_wise_e_p => {
            res.status(200).send({
                message: 'Retrieved categoryWiseEP Successfully',
                data: category_wise_e_p,
            });
        })
        .catch(err => {
...
// Update categoryWiseEP
exports.update = (req, res) => {
    categoryWiseEP.update({
            yearId: req.body.yearId,
            stateId: req.body.stateId,
            districtId: req.body.districtId,
            blockId: req.body.blockId,
            ...
            where: {
                id: req.params.categoryWiseEPId
            }
        })
        .then((category_wise_e_p) => {
            res.status(200).send({
                message: "categoryWiseEP updated successfully",
                data: category_wise_e_p,
            });
        })
        .catch(err => {
...
// Delete categoryWiseEP
exports.delete = (req, res) => {
    categoryWiseEP.destroy({
            where: {
                id: req.params.categoryWiseEPId
            }
        })
        .then(() => {
            res.status(200).send({
                message: 'categoryWiseEP deleted successfully',
            });
        })
        .catch(err => {
            res.status(500).send({
...
exports.firstChart = async (req, res) => {
    ...
    categoryWiseEP.findAll({
        ...
        .then(category_wise_f_ep => {
            ...
            category_wise_f_ep.forEach(category_wise_e_p => {
                for (var i = 0; i < panchayatId.length; i++) {
                    if (category_wise_e_p.panchayatId == panchayatId[i]) {
                        for (var j = 0; j < params.length; j++) {
                            data[panchayatId[i]].push(category_wise_e_p[params[j]]);
                        }
                    }
                }
            })
...
exports.upload = async (req, res) => {
    try {
        ...
        readXlsxFile(path).then((rows) => {
            ...
            console.log(years);
            categoryWiseEP.bulkCreate(years)
                .then(() => {
                    res.status(200).send({
